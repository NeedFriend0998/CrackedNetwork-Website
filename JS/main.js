...
// Urutan halaman sesuai navbar (tanpa #legal, #staff-detail, dll. yang spesial)
const pageOrder = ['home', 'gamemodes', 'wiki', 'rules', 'vote', 'store', 'discord', 'forums', 'plugins', 'donate'];

// Ambil index halaman saat ini
function getCurrentPageIndex() {
    const active = document.querySelector('.page.active');
    return active ? pageOrder.indexOf(active.id) : -1;
}

// Navigasi ke halaman dengan arah slide
let isSwiping = false; // mencegah double swipe
...
function swipeToPage(targetId, direction) {
    ...
    // Siapkan halaman target: tampilkan dan posisikan di luar layar
    targetPage.classList.add('swipe-animating');
    if (direction === 'next') {
    ...
    // Halaman saat ini: set absolute
    currentPage.classList.add('swipe-animating');
    currentPage.classList.remove('active'); // sementara nonaktifkan active

    // Trigger reflow agar transisi berjalan
    void targetPage.offsetWidth;

    // Jalankan animasi: target masuk ke posisi normal, current keluar
    if (direction === 'next') {
    ...
    const onTransitionEnd = (e) => {
        if (e.target !== targetPage) return; // hanya tangani target
        targetPage.removeEventListener('transitionend', onTransitionEnd);

        // Bersihkan kelas animasi
        currentPage.classList.remove('swipe-animating', 'swipe-exit-left', 'swipe-exit-right');
        currentPage.style.display = ''; // kembalikan ke display none (diatur CSS)
        targetPage.classList.remove('swipe-animating', 'swipe-enter-right', 'swipe-enter-left');
        targetPage.style.transform = '';

        // Aktifkan halaman target secara normal
        targetPage.classList.add('active');

        // Update navbar active link (opsional, jika kamu sudah punya fungsi)
        const navLink = document.querySelector(`.nav-link[data-page="${targetId}"]`);
        if (navLink) {
...
function handleSwipeEnd(e) {
    ...
    // Hanya jika dominan horizontal dan memenuhi syarat jarak & waktu
...
// Pasang event di seluruh halaman (bisa di document atau .pages)
...
// Dukungan mouse drag (opsional untuk desktop)
