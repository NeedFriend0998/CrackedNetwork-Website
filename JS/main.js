...
if (!window.isLowEnd) {
    ...
    // Update Lenis saat halaman berganti
    window.addEventListener('pageChanged', () => {
    ...
    bedwars: [
        {
            ...
            price: "12 USD",
            badge: "cheapest",
            features: ["Have access to Vip Cosmetics"]
        },
        {
            id: "1561294",
            name: "Premium",
            ...
            price: "18 USD",
            badge: null,
            features: ["Have access to Premium and Vip Cosmetics"]
        },
        {
            id: "1561297",
            name: "Legend",
            ...
            price: "24 USD",
            badge: null,
            features: ["Have access to Premium, Vip, and Legend Cosmetics"]
        }
    ],
    special: [
        {
...
const staffTeam = [
    ...
    },
    {
        ...
        discord: "-",
        status: "Position Open",
        responsibilities: ["General staff duties", "Player support & moderation"],
        tags: ["Open Position"],
        socials: {}
    },
    {
...
if (!prefersReducedMotion && !window.isLowEnd) {
    resizeCanvas();
    createParticles();
    drawParticles();
    window.addEventListener('resize', () => { resizeCanvas(); createParticles(); });
...
// ============================================================
const SEO_DATA = {
    "/home": { title: "CrackedNetwork | Non Pay To Win Minecraft Server", desc: "Join CrackedNetwork..." },
    "/gamemodes": { title: "Gamemodes | CrackedNetwork Minecraft Server", desc: "Explore all available gamemodes..." },
    "/wiki": { title: "Wiki | CrackedNetwork Help & Information", desc: "Learn how CrackedNetwork works..." },
    "/rules": { title: "Server Rules | CrackedNetwork", desc: "Read the official CrackedNetwork server rules..." },
    "/vote": { title: "Vote | CrackedNetwork Minecraft Server", desc: "Vote for CrackedNetwork..." },
    "/store": { title: "Store | CrackedNetwork Official Webstore", desc: "Purchase ranks, coins, cosmetics..." },
    "/discord": { title: "Discord | CrackedNetwork Community", desc: "Join the official CrackedNetwork Discord server..." },
    "/forums": { title: "Forums | CrackedNetwork Community Discussions", desc: "Join discussions with the CrackedNetwork community..." },
    "/plugins": { title: "Plugins | CrackedNetwork Server Information", desc: "View the plugins and systems running on CrackedNetwork..." },
    "/donation": { title: "Donation | Support CrackedNetwork", desc: "Support CrackedNetwork with donations..." },
    "/legal": { title: "Legal Information | CrackedNetwork", desc: "Read the official CrackedNetwork legal documents..." },
    "/terms-of-service": { title: "Terms of Service | CrackedNetwork", desc: "Read the CrackedNetwork Terms of Service..." },
    "/privacy-policy": { title: "Privacy Policy | CrackedNetwork", desc: "Learn how CrackedNetwork collects, stores, and protects your personal data..." }
};
...
function updateSEO(path) {
    ...
    document.querySelector('link[rel="canonical"]').setAttribute("href", "https://crackednetwork.netlify.app" + path);
}

// ============================================
// SWIPE NAVIGATION SYSTEM (FINAL, NO DUPLICATE)
// ============================================
...
function swipeToPage(targetId, direction) {
    ...
    const cleanup = () => {
        currentPage.classList.remove('swipe-active', 'swipe-transition', 'swipe-exit-left', 'swipe-exit-right');
        currentPage.style.display = '';
        currentPage.style.transform = '';
        currentPage.classList.remove('active');

        targetPage.classList.remove('swipe-transition', 'swipe-enter-right', 'swipe-enter-left', 'swipe-to-center');
...
const minSwipeDistance = 60, maxSwipeTime = 400, maxVerticalMovement = 50;

function handleSwipeStart(e) {
    if (e.target.closest('button, a, input, textarea, select, .wiki-tab, .nav-link, iframe, [data-no-swipe]')) return;
    const p = e.touches ? e.touches[0] : e;
    touchStartX = p.clientX; touchStartY = p.clientY; touchStartTime = Date.now();
    isDragging = true;
}
...
function handleSwipeMove(e) {
    ...
    const p = e.touches ? e.touches[0] : e;
    if (Math.abs(p.clientY - touchStartY) > maxVerticalMovement && Math.abs(p.clientY - touchStartY) > Math.abs(p.clientX - touchStartX)) {
        isDragging = false;
    }
}
...
function handleSwipeEnd(e) {
    ...
    const dx = p.clientX - touchStartX, dy = p.clientY - touchStartY, dt = Date.now() - touchStartTime;
    if (Math.abs(dx) < minSwipeDistance || dt > maxSwipeTime || Math.abs(dy) > Math.abs(dx)) return;

    const idx = getCurrentPageIndex();
    if (idx === -1) return;
    if (dx < 0 && idx + 1 < pageOrder.length) swipeToPage(pageOrder[idx + 1], 'next');
    else if (dx > 0 && idx - 1 >= 0) swipeToPage(pageOrder[idx - 1], 'prev');
}
...
if (swipeArea) {
    ...
    document.addEventListener('mousemove', e => { if (e.buttons === 1) handleSwipeMove(e); });
    document.addEventListener('mouseup', e => { if (isDragging) handleSwipeEnd(e); });
...
// Integrasi klik navigasi dengan swipe animation
    ...
    document.querySelectorAll('[data-page]').forEach(link => {
        ...
        link.addEventListener('click', function(e) {
            ...
            if (currentIdx !== -1 && targetIdx !== -1 && currentIdx !== targetIdx) {
                e.preventDefault();
                swipeToPage(pageId, targetIdx > currentIdx ? 'next' : 'prev');
            }
        });
    });
});

console.log('✅ All systems initialized (Lenis, Swipe, SEO, Store, etc.)');